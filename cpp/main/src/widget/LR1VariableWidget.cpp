...
LR1VariableWidget::LR1VariableWidget(QWidget *parent)
	: QWidget(parent), variableValid(false), x(0), y(0), mousePressed(false) {
}

LR1VariableWidget::~LR1VariableWidget() {
}

void LR1VariableWidget::setVariableAndPoint(
...
void LR1VariableWidget::translateDefault() {
	x = 0;
	y = 0;
...
void LR1VariableWidget::paintEvent(QPaintEvent *event) {
...
void LR1VariableWidget::mousePressEvent(QMouseEvent *event) {
	mousePressed = true;
	lastMouseX = event->x();
	lastMouseY = event->y();
}

void LR1VariableWidget::mouseReleaseEvent(QMouseEvent *event) {
	mousePressed = false;
}

void LR1VariableWidget::mouseMoveEvent(QMouseEvent *event) {
	if (!mousePressed) {
		return;
	}
	float x = event->x();
...
void LR1VariableWidget::paintComputeFirst(const PaintContext &ctx) {
	paintProductionOrder(ctx);
	paintFirstTable(ctx);
}

void LR1VariableWidget::paintComputeItemClosure(const PaintContext &ctx) {
	paintLR1ItemClosure(ctx, 0, 0);
}

void LR1VariableWidget::paintLRGenerateAutomaton(const PaintContext &ctx) {
	paintLR1ItemClosure(ctx, 0, 0);
	paintAutomatonTable(ctx, itemClosureWidth + 16, 0);
}

void LR1VariableWidget::paintProductionOrder(const PaintContext &ctx) {
...
void LR1VariableWidget::paintFirstTable(const PaintContext &ctx) {
...
void LR1VariableWidget::paintLR1ItemClosure(const PaintContext &ctx, int left,
											int top) {
	left += 8;
	top += 8;
	int height = ctx.normalFontMetrics->height();
...
void LR1VariableWidget::paintAutomatonTable(const PaintContext &ctx, int left,
											int top) {
...
void LR1VariableWidget::computeItemLayout(const PaintContext &ctx,
...
void LR1VariableWidget::computeItemWidthAndHeight(const PaintContext &ctx,
...
int LR1VariableWidget::computeItemWidth(const PaintContext &ctx,
										ipc::LRItem item) {
...
QRect LR1VariableWidget::computeProductionCellBounding(const PaintContext &ctx,
...
QString LR1VariableWidget::getItemString(const ipc::LRItem &item) {
